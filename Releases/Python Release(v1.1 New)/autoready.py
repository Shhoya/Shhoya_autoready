...
logo = """
	...
	|                                                /____/    |
	|                                                          |
	|  Made by Shh0ya of the Cyb3r1                    v 1.1   |
	 ==========================================================
"""
### Directory check###
def dirchk():
...
def all():
	...
	find_input=raw_input("[*] Find as app name ?(y or n) : ")
	if find_input=='y' or find_input=='Y':
		find_str=raw_input("[*] find string : ")
		print "\n"
		os.system(adb_s+"find /data/app/* -maxdepth 0 -name \"*"+find_str+"*\" -exec basename {} \;' > find_list.txt")
		shell=read_ready("find_list.txt","/sdcard/Download/Autoready_apk/")
		os.system(shell)
...
def data():
	...
	if find_input=='y' or find_input=='Y':
		find_str=raw_input("[*] find string : ")
		os.system(adb_s+"find /data/data/* -maxdepth 0 -name \"*"+find_str+"*\" -exec basename {} \;' > find_dlist.txt")
		print "\n"
		shell=read_ready("find_dlist.txt","/sdcard/Download/Autoready_data/")
...
def searchUniq():
	...
	for root, dirs, files in os.walk('./app_data'):
		for fname in files:
			...
			try :
				handle = open(full_fname, 'rb+')
			except IOError:
				print 'IOError : File path is invalid'
				sys.exit()
			for line in handle.readlines():
				for matchobj in jumin.finditer(line):
					print 'Suspicious File : '+full_fname+'\n'+'\tResident registration number : \"'+matchobj.group()+'\"\n'
					matchcount += 1
				for matchobj in jumin2.finditer(line):
					print 'Suspicious File : '+full_fname+'\n'+'\tResident registration number : \"'+matchobj.group()+'\"\n'
					matchcount += 1
...
def help():
	...
	print " -s, --search       Retrieves unique identification information"
